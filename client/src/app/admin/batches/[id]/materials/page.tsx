...
const TYPE_ICONS: Record<string, React.ReactNode> = {
  ...
  doc:   <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>,
...
interface BatchDetail { _id: string; name: string; mode: string; courseId: { _id: string; title: string } | string; }

function ResourceForm({ f, setF, err }: { f: Partial<Resource>; setF: (v: Partial<Resource>) => void; err: string }) {
  return (
...
export default function BatchMaterialsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/batches/${batchId}`, { headers: authHeader })
      .then(r => r.json())
      .then(json => { if (json.success) setBatch(json.data); })
      .catch(() => {});
  }, [batchId]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BASE}/resources?batchId=${batchId}&limit=200`, { headers: authHeader });
      const json = await res.json();
      setRes(json.data?.resources ?? []);
  ...
  const courseName = batch?.courseId
    ? (typeof batch.courseId === "object" ? batch.courseId.title : "")
    : "";
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/resources`, {
        ...
        body: JSON.stringify({ ...form, batchId }),
      });
      const json = await res.json();
  ...
  const openEdit = (r: Resource) => {
    setEditItem(r);
    setEForm({ title: r.title, description: r.description ?? "", type: r.type, url: r.url, section: r.section ?? "General", sortOrder: r.sortOrder, duration: r.duration ?? "", isPublic: r.isPublic });
    setSaveError("");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/resources/${editItem._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...authHeader },
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`${BASE}/resources/${deleteItem._id}`, { method: "DELETE", headers: authHeader });
      const json = await res.json();
      if (!json.success) throw new Error(json.message ?? "Failed");
  ...
  const modeLabel = (m: string) => m === "recorded" ? "Recorded" : m === "one_on_one" ? "One-on-One" : "Live";
    ...
    <AdminGuard>
      <div className={styles.inner}>
        ...
        <div className={styles.main}>
          <Topbar title="Batch Materials" />
          <div className={styles.content}>

            {/* Breadcrumb */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, fontSize: 13, color: "#6B7280" }}>
              <Link href="/admin/batches" style={{ color: "#6B7280" }}>Batches</Link>
              <span>›</span>
              <span style={{ color: "#111827", fontWeight: 600 }}>{batch?.name ?? "Loading…"}</span>
              <span>›</span>
              <span>Materials</span>
            </div>
            ...
            {/* Batch info card */}
            {batch && (
              <div style={{ background: "linear-gradient(135deg,#1a1a2e 0%,#3a3a5c 100%)", borderRadius: 14, padding: "18px 24px", marginBottom: 20, display: "flex", alignItems: "center", gap: 20, color: "#fff" }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, fontFamily: "var(--font-heading)" }}>{batch.name}</div>
                  <div style={{ fontSize: 13, opacity: 0.75, marginTop: 4 }}>{courseName} · {modeLabel(batch.mode)}</div>
                </div>
                <span style={{ marginLeft: "auto" }}>
                  <Badge variant={batch.mode === "live" ? "green" : batch.mode === "recorded" ? "blue" : "gray"}>
            ...
            <div className={styles.card}>
              ...
              {loading ? (
                ...
                <div className={styles.empty}>
                  ...
                  <div className={styles.emptyText}>No materials for this batch yet. Add videos, PDFs, links or live class links.</div>
                </div>
              ) : (
                <table className={styles.table}>
                  <thead>
                    <tr><th>Type</th><th>Title</th><th>Section</th><th>Duration</th><th>Visibility</th><th>Actions</th></tr>
                  </thead>
                  <tbody>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Add Material to Batch">
